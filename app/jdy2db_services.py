...
from sqlalchemy import inspect, Table, Column, String, text, JSON, DateTime, Text, BigInteger, Float, MetaData
from sqlalchemy.dialects.mysql import insert, LONGTEXT
from sqlalchemy.exc import SQLAlchemyError
from sqlalchemy.orm import Session, joinedload
from sqlalchemy.sql.sqltypes import TypeEngine, Integer, Boolean

from app.db2jdy_services import (
    get_dynamic_session, get_dynamic_engine
)
from app.jdy_api import FormApi, DataApi
from app.models import (SyncTask, FormFieldMapping, ConfigSession)
from app.utils import (retry, get_sql_type, convert_to_pinyin, get_column_name,
                       log_sync_error, TZ_UTC_8)
...
def handle_webhook_data(config_session: Session, payload: dict, task_config: SyncTask, api_client: FormApi,
                        table_param: str = None):
    """
    处理 Webhook 推送的数据（已重构和优化）。
    ...
    # --- REFACTOR: 3. 获取动态引擎和元数据 ---
    try:
        if not task_config.database:
            # 如果 database 没有被 eager load (理论上 routes.py 应该做了)
        ...
        # 初始化此引擎的表缓存
        if engine_url_key not in inspected_tables_cache:
            inspected_tables_cache[engine_url_key] = {}
    ...
    # --- REFACTOR: 4. 自动保存 App ID 和 Entry ID (如果缺少) ---
    if not app_id or not entry_id:
        if op == 'form_update' or (op in ('data_create', 'data_update') and data):
            payload_app_id, payload_entry_id = None, None
            if isinstance(data, dict):
                payload_app_id = data.get('appId')
                payload_entry_id = data.get('entryId')
            if not all([payload_app_id, payload_entry_id]):
                payload_app_id = payload.get('appId')
                payload_entry_id = payload.get('entryId')

            if all([payload_app_id, payload_entry_id]):
                try:
                    logger.info(f"任务 {task_config.id} 缺少 App/Entry ID，正在从 Webhook 自动填充...")
                    task_config.app_id = payload_app_id
                    task_config.entry_id = payload_entry_id
                    config_session.commit()
                    ...
                    # 如果填充失败 (例如唯一约束)，我们无法继续，因为映射依赖 App/Entry ID
                    return
            else:
    ...
    try:
        ...
        if op == 'form_update':
            ...
            # 立即同步映射表
            create_or_update_form_fields_mapping(config_session, task_config, api_client, form_name)

            # 获取或创建表
            table = get_table_if_exists(table_name, dynamic_engine)
            if table is None:
                table = get_or_create_table_from_schema(table_name, data, task_config, dynamic_engine, dynamic_metadata)

            # 更新表结构 (处理添加、删除、重命名、类型变更)
            table = handle_table_schema_from_form(config_session, table, data, task_config, dynamic_engine,
                                                  dynamic_metadata)
        ...
        # 3.2 其他数据操作
            ...
            exists = config_session.query(FormFieldMapping).filter_by(
                task_id=task_config.id  # (修复) 按 task_id 过滤
            ).first()
            if not exists:
            ...
            if table is None and op != 'data_remove':  # 只有在需要写入数据时才创建表
                ...
                table = get_or_create_table_from_data(table_name, [data], task_config, dynamic_engine, dynamic_metadata)

            # 如果是 data_remove 且表不存在，则无需操作
            if table is None and op == 'data_remove':
                logger.warning(f"警告: 收到删除操作，但表 '{table_name}' 不存在。正在跳过...")
                return
            ...
            with get_dynamic_session(task_config) as target_session:
                try:
                    if op in ('data_create', 'data_update', 'data_recover'):
                        # 确保表结构与数据兼容 (主要处理新增列)
                        table = handle_table_schema_from_data(table, [data], task_config, dynamic_engine,
                                                              dynamic_metadata)
                        upsert_data(target_session, table, data, task_config)

                    elif op == 'data_remove':
                        delete_data(target_session, table, data, task_config)
...
# --- 任务与映射管理 ---
# (create_sync_task 已被 routes.py Webhook 逻辑取代，不再需要)
# def create_sync_task(session: Session, app_id: str, entry_id: str,
#                      department_name: str, table_name: str) -> SyncTask:
#     """
#     在配置库中创建一个新的同步任务。
#     :param session: SQLAlchemy Session
#     :param app_id: 应用 ID
#     :param entry_id: 表单 ID
#     :param department_name: 部门名称
#     :param table_name: 预先根据优先级确定的目标表名
#     :return: 创建的 SyncTask 对象
#     """
#     logger.info(f"为 app_id={app_id}, entry_id={entry_id} 自动创建同步任务, 目标表: {table_name}...")
#
#     try:
#         sync_time = datetime.strptime(Config.GLOBAL_SYNC_TIME, '%H:%M').time()
#     except ValueError:
#         logger.warning(f"GLOBAL_SYNC_TIME ('{Config.GLOBAL_SYNC_TIME}') 格式无效，将 sync_time 设置为 None。")
#         sync_time = None  # 设置为 None 或一个默认时间，如 00:00
#
#     new_task = SyncTask(
#         app_id=app_id,
#         entry_id=entry_id,
...
#         column_use_label_pinyin=False,
#         send_error_log_to_wecom=False,
#         wecom_robot_webhook_url=None
#     )
#
#     try:
#         session.add(new_task)
#         session.commit()
#         logger.info(f"新任务已创建: {table_name} (租户: {department_name})")
#         return new_task
#     except SQLAlchemyError as e:
#         session.rollback()
#         logger.error(f"创建同步任务失败: {e}", exc_info=True)
#         log_sync_error(
#             app_id=app_id, entry_id=entry_id, department_name=department_name,
#             error=e, extra_info="Failed to auto-create sync task"
#         )
#         raise  # 重新抛出异常，让上层知道创建失败


@retry(max_retries=3, delay=3)
def create_or_update_form_fields_mapping(config_session: Session, task_config: SyncTask,
    ...
    try:
        ...
        if not api_widgets:
            logger.warning(f"警告: API 未返回 {app_id}/{entry_id} 的任何字段信息，跳过不予处理。")
        ...
        existing_mappings_query = config_session.query(FormFieldMapping).filter_by(
            task_id=task_id
        )
        ...
        for widget in api_widgets:
            ...
            # 统一计算列名
            column_name = get_column_name(widget, task_config.label_to_pinyin)
            ...
            if mapping_to_update:
                ...
                new_mapping = FormFieldMapping(
                    task_id=task_id,  # (修复)
                    # app_id=app_id, (移除, 已被 task_id 取代)
                    # entry_id=entry_id, (移除)
                    form_name=form_name,
...
def get_table_if_exists(table_name: str, engine) -> Table | None:
    """
    如果表存在，则从缓存或数据库加载表定义。
    --- REFACTOR: 6. 动态引擎 ---
    """
    engine_url_key = str(engine.url)
    global inspected_tables_cache

    if engine_url_key not in inspected_tables_cache:
        inspected_tables_cache[engine_url_key] = {}

    if table_name in inspected_tables_cache[engine_url_key]:
        return inspected_tables_cache[engine_url_key][table_name]
    ...
    try:
        ...
        if inspector.has_table(table_name):
            ...
            inspected_tables_cache[engine_url_key][table_name] = table
            logger.info(f"从数据库 ({engine_url_key}) 加载表定义: {table_name}")
            return table
    except Exception as e:
        logger.error(f"检查或加载表 {table_name} 定义时出错 ({engine_url_key}): {e}", exc_info=True)
        inspected_tables_cache[engine_url_key].pop(table_name, None)
    return None


def get_or_create_table_from_data(table_name: str, data_samples: list[dict], task_config: SyncTask, engine,
                                  metadata) -> Table:
    """
    获取或创建数据表。如果表不存在，则根据数据样本动态创建。
    --- REFACTOR: 7. 动态引擎/元数据 ---
    """
    table = get_table_if_exists(table_name, engine)
    if table is not None:
        return table

    logger.info(f"表 '{table_name}' 不存在，正在根据数据样本创建...")
    try:
        column_defs, column_comments = get_column_schema(data_samples, task_config)
        columns = [Column(name, col_type, comment=column_comments.get(name)) for name, col_type in column_defs.items()]
        ...
        for i, col_def in enumerate(columns):
            if col_def.name == '_id':
                # 显式添加 unique=True
                final_columns.append(Column('_id', column_defs['_id'], unique=True, comment=column_comments.get('_id')))
        ...
        engine_url_key = str(engine.url)
        inspected_tables_cache[engine_url_key][table_name] = table  # 加入缓存
        return table
    except Exception as e:
...
def get_or_create_table_from_schema(table_name: str, form_schema_data: dict, task_config: SyncTask, engine,
                                    metadata) -> Table:
    """
    ...
    try:
        ...
        for widget in widgets:
            col_name = get_column_name(widget, use_label_pinyin=task_config.label_to_pinyin)
            jdy_type = widget.get('type')
            jdy_label = widget.get('label')
            if col_name and col_name not in added_col_names:  # 确保列名有效且未重复添加
                sql_type = get_sql_type(jdy_type, None)  # 从 schema 创建时不依赖样本值
...
def get_column_schema(data_samples: list[dict], task_config: SyncTask) -> (dict, dict):
    """根据一批数据样本分析并生成所有字段的列定义（已重构）"""
    column_types = {}
    column_comments = {}
    ...
    for key in all_keys:
        ...
        sample_value = next((d[key] for d in data_samples if isinstance(d, dict) and key in d and d[key] is not None),
                            None)
        ...
        if key == '_id':
            ...
            if mapping_info:
                # (修复) 从 mapping_info 推断列名
                db_col_name = get_column_name(
                ...
                sql_type_instance = get_sql_type(jdy_type, sample_value)  # 直接获取实例
            else:
                # 如果映射不存在，则退化为旧逻辑
        ...
        if db_col_name in column_types:
            ...
            if isinstance(sql_type_instance, (LONGTEXT, Text)):
                ...
                column_types[db_col_name] = sql_type_instance
            elif isinstance(sql_type_instance, DateTime) and not isinstance(current_type_inst, (LONGTEXT, Text, JSON)):
                column_types[db_col_name] = sql_type_instance
...
def handle_table_schema_from_data(table: Table, data_batch: list[dict], task_config: SyncTask, engine,
                                  metadata) -> Table:
    """
    ...
    try:
        ...
        for m in mappings.values():
            ...
            db_col_name = get_column_name(widget_dict, task_config.label_to_pinyin)
        ...
        with engine.connect() as connection:
            with connection.begin() as transaction:
                ...
                for col_name, col_info in all_new_columns.items():
                    ...
                    sql_type_instance = get_sql_type(col_info["type"], col_info['value'])
                    # 获取类型的 SQL 字符串表示
    ...
    engine_url_key = str(engine.url)
    if engine_url_key in inspected_tables_cache:
        inspected_tables_cache[engine_url_key].pop(table.name, None)

    new_table = get_table_if_exists(table.name, engine)
    if new_table:
...
def _is_type_different(existing_sqlalch_type: TypeEngine, expected_sqlalch_type: TypeEngine) -> bool:
...
def handle_table_schema_from_form(config_session: Session, table: Table, data: dict, task_config: SyncTask, engine,
                                  metadata):
    """
    处理表单结构更新事件 (form_update)，处理列的添加、删除、重命名和类型变更。
    --- REFACTOR: 10. 动态引擎/元数据 ---
    """
    ...
    try:
        ...
        if not inspector.has_table(table_name):
            ...
            return get_or_create_table_from_schema(table_name, data, task_config, engine, metadata)
        ...
        # 处理来自 API 的 widgets
        for widget in widgets:
            db_col_name = get_column_name(widget, task_config.label_to_pinyin)
            if db_col_name:
                sql_type_instance = get_sql_type(widget.get('type'), None)
            ...
            if new_col_name and old_col_name != new_col_name:
                ...
                if expected_info:
                    ...
                    if existing_col_info:
                        ...
                        if _is_type_different(existing_col_info['type'], expected_info['sql_type']):
                            cols_to_modify.append((old_col_name, expected_info['widget'], expected_info['sql_type']))
                        ...
                        for old_name, new_name, widget, sql_type_inst in cols_to_rename:
                            ...
                            if existing_col_info:
                                ...
                                if new_name in [c['name'] for c in temp_inspector.get_columns(table_name) if
                                                c['name'] != old_name]:
                                    logger.error(f"无法重命名列 '{old_name}' 为 '{new_name}'，因为目标列名已存在。")
                                    continue  # 跳过此重命名
        ...
        if ddl_executed:
            ...
            new_table = get_table_if_exists(table_name, engine)
            if new_table:
...
def clean_data_for_db(table: Table, data: dict, task_config: SyncTask) -> dict:
    """根据表结构和动态配置清理数据（已重构）"""
...
@retry()
def upsert_data(session: Session, table: Table, data: dict, task_config: SyncTask):
    """插入或更新单条数据 (使用 ON DUPLICATE KEY UPDATE 增强)"""
    cleaned_data = clean_data_for_db(table, data, task_config)
    if not cleaned_data:
...
@retry()
def delete_data(session: Session, table: Table, data: dict, task_config: SyncTask):
    """根据 _id 删除数据"""
    data_id = data.get('_id')
    if not data_id:
        logger.warning("警告：删除操作的数据中缺少 '_id'。数据: %s", json.dumps(data, ensure_ascii=False, default=str))
        return
...
@retry()
def sync_historical_data(task_config: SyncTask, api_client: DataApi):
    ...
    if not all([app_id, entry_id, table_name]):
        ...
        log_sync_error(task_config=task_config, error=ValueError("Missing App ID/Entry ID/Table Name for full sync"))
        return
    ...
    try:
        ...
        while True:
            ...
            if first_batch and table is None:
                ...
                table = get_or_create_table_from_data(table_name, data_list, task_config, dynamic_engine,
                                                      dynamic_metadata)
                table = handle_table_schema_from_data(table, data_list, task_config, dynamic_engine, dynamic_metadata)
                first_batch = False
            ...
            try:
                with get_dynamic_session(task_config) as target_session:
                    for item in data_list:
                        upsert_data(target_session, table, item, task_config)
                ...
                logger.error(f"任务 {table_name}: 处理单条数据时发生意外错误 (last_data_id={last_data_id}): {item_err}",
                             exc_info=True)
                target_session.rollback()
                raise item_err
        ...
        config_session.query(SyncTask).filter_by(id=task_config.id).update(
            {"sync_status": 'SUCCESS', "last_sync_time": datetime.now(TZ_UTC_8)}
        )
        config_session.commit()
        ...
        try:
            ...
            config_session.query(SyncTask).filter_by(id=task_config.id).update(
                {"sync_status": 'FAILED', "last_sync_time": sync_start_time}  # 使用开始时间标记失败时间点
            )
            config_session.commit()
            logger.info(f"任务 {table_name} 状态已更新为 FAILED。")
